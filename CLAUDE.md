...
### ファイル行数（`noExcessiveLinesPerFile`）

`npm run check` は**ファイル行数**の `noExcessiveLinesPerFile`（閾値300・`warn`）も含む。1ファイルに機能を盛りがちな傾向への**粗い signal**で、認知的複雑度と同じ思想で扱う——閾値超え＝設計を見直す合図であって、数値を割ること自体は目的ではない。

- この警告への正しい応答は、責務ごとに**別モジュール（別ファイル）へ分割**して見通しとコンテキストを改善すること。無意味に行を詰めて数値だけ通すのはしない（上の「機械的な分割もしない」と同じ精神で、逆向きにも適用）。
- 本質的に1ファイルにまとまっているべき理由があるなら、`// biome-ignore lint/style/noExcessiveLinesPerFile: <なぜ必要か>` で理由を明記して抑制する。

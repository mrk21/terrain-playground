...
export function attachGestures(canvas: HTMLCanvasElement, target: GestureTarget): () => void {
  ...
  // 2 本指の重心・間隔・角度を取り出す。
  const twoFinger = (): { cx: number; cy: number; dist: number; angle: number } => {
  ...
  const onPointerMove = (e: PointerEvent): void => {
    ...
    if (mode === 'drag' && pointers.size === 1) {
      ...
      if (dy !== 0) target.onPinch?.(Math.exp(dy * ONE_FINGER_ZOOM_PER_PX), zoomFx, zoomFy)
    } else if (mode === 'pinch' && pointers.size >= 2) {
      const g = twoFinger()
      pinchMoved += Math.hypot(g.cx - pinchCx, g.cy - pinchCy) + Math.abs(g.dist - prevDist)
  ...
  const onPointerUp = (e: PointerEvent): void => {
    ...
    if (prevMode === 'pinch' && wasTwo) {
      const quick = e.timeStamp - pinchStartTime <= TAP_MAX_MS && pinchMoved <= TAP_SLOP
      if (quick) {
